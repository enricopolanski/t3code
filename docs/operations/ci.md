...
- `.github/workflows/ci.yml` runs `vp check` (lint + typecheck), `vpr typecheck`, and
  `vp run test --package-concurrency 2 --package-max-workers 2` on pull requests and
  pushes to `main`. The test command starts the single-worker server suite alongside a
  bounded non-server package lane, preserving the full workspace test coverage while
  avoiding CPU oversubscription on the 8-vCPU runner.
